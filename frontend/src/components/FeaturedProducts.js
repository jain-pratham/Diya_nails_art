  ...
  return (
    <section className="w-full py-14 md:py-20 bg-white">
      <div className="max-w-[1410px] mx-auto px-4 sm:px-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-end mb-8 md:mb-10">
          <h2 className="text-2xl sm:text-3xl md:text-[34px] text-[#333333] font-normal tracking-tight">{title}</h2>
          ...
          {loading
            ? Array.from({ length: limit }).map((_, index) => (
                <div key={index} className="w-full flex flex-col cursor-pointer group/card">
                  <div className="relative w-full aspect-square rounded-2xl overflow-hidden mb-4 bg-[#F8F1E7] animate-pulse" />
